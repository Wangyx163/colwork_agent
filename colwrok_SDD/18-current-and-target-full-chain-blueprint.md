...
- AMC-A 带 action item 人工标注，654 场中文会议，CC BY 4.0 —— 是唯一合适的公开语料。**但 ModelScope 原始文件需要鉴权（匿名访问 401）**，HuggingFace 上只有 AliMeeting（ASR 语料）镜像，不含行动项标注。取数需要 ModelScope 账号 token。
- 项目自带的 `fixtures/ai_p0_extraction_cases.json` 目前只有 2 条、且 `prediction == expected`，`positive_sentence_rate` 为 1.0 —— 与真实语料的稀疏正例分布完全不同，**不能用来声称任何抽取质量**。评测工具会把这个分布直接打印出来，避免误读。
- 加载器同时支持 AMC-A JSONL 与项目自标注 JSON；`replay_extractor` 可以对已保存的抽取结果重复计分，让盲测集零 Token 成本可复算。
...
2. **上游 Gate**：每个人的问题清单是否都必须先由会议负责人验收，才允许子恒整理？我建议保留，展示 HITL 和 lineage，但 UI 可做批量验收。
