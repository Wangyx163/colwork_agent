...
- dev 里有 10/65 场**完全没有行动项**，是天然的误报对照：那里的任何预测都无歧义地是假警报，而 F1 会把它们藏起来（无真正例可贡献）。因此单列 `false_alarm_rate_on_empty_meetings`。
...
**定义无关、因而可直接宣称的两项**：引文可定位率 1.0（零捏造）、零行动项会议保持干净（零模型地板误报率 0.3）。
